...
//Backprojects matterport or other depthmans
...
//Transforms the pointclouds to compensate the poses.
...
__global__ void transform2d(float *points3d_after, float fov_scale, float sensor_scale)
{
 int x = blockIdx.x * TILE_DIM + threadIdx.x;
  int y = blockIdx.y * TILE_DIM + threadIdx.y;
  int w = gridDim.x * TILE_DIM;
  ...
  for (int j = 0; j < TILE_DIM; j+= BLOCK_ROWS)
  {
     ...
     float z = points3d_after[(ih * w + iw) * 3 + 2];
      ...
      points3d_after[(ih * w + iw) * 3 + 0] = x;

      float x2 = fov_scale * x;
      if ((x2 > 0) && (y < x2 * 1.1) && (y > -x2 * 1.1) && (z < x2 * 1.1) && (z > -x2 * 1.1)) {
          points3d_after[(ih * w + iw) * 3 + 0] = x / sensor_scale;
          points3d_after[(ih * w + iw) * 3 + 1] = y / (x2 + 1e-5) / sensor_scale;
...
void render(int n, int h,int w, int oh, int ow, unsigned char * img, float * depth,float * pose, unsigned char * render, float * depth_render, float fov, float sensor_scale){
    //int ih, iw, i, ic;
    //printf("inside cuda code %d\n", depth);
    
    //printf("scale %d\n", s);

    printf("sensor scale %9.6f\n", sensor_scale);
    ...
    for (idx = 0; idx < n; idx ++) {
        ...
        transform2d<<<dimGrid, dimBlock>>>(d_3dpoint_after, fov_scale, sensor_scale);
